...
public class TenantLifecycleController {
    ...
    ApiResponse<Map<String, Object>> sign(@PathVariable Long partyId, @PathVariable Long admissionId) {
        ...
        BigDecimal securityDeposit = admission.get("security_deposit") != null ? decimal(admission.get("security_deposit")) : BigDecimal.ZERO;
        occupancyService.assign(currentUser.organizationId(), currentUser.userLoginId(),
                new BedAssignRequest(partyId, bedId, moveIn, null,
                        securityDeposit.compareTo(BigDecimal.ZERO) > 0 ? securityDeposit : null, null));
        ...
        if (securityDeposit.compareTo(BigDecimal.ZERO) > 0) {
            jdbc.update("INSERT INTO invoice_item(invoice_id,item_type_id,description,amount,created_at,updated_at) VALUES(?,?,?,?,?,?)",
                    invoiceId, "SECURITY_DEPOSIT", "Security Deposit (one-time)", securityDeposit, LocalDateTime.now(), LocalDateTime.now());
        }
