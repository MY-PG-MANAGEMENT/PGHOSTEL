...
1. Select the target organization in the wizard (or set `org_code` in the CSV to target one by code).
2. Upload **facilities_200.csv** -> **creates the property `Sunrise PG` (code `PROP-SUNRISE`)** plus
...
## Org + facility codes
- `org_code` (both files, optional): target org by its code; blank = the org selected in the wizard.
- The property is created with code `PROP-SUNRISE`; every floor/room/bed carries an explicit code
