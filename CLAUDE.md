...
- Categories (`FOOD`, `SALARY`, `ELECTRICITY`, `MAINTENANCE`, `LAUNDRY`, `TRANSPORT`, `RENT`, `DEPOSIT_REFUND`, `OTHERS`) and payment methods are string constants — no master tables. Approval is the `status` column (`PENDING`/`APPROVED`/`REJECTED`/`PAID`); only `PENDING` can transition. `POST /` creates `APPROVED` unless `requiresApproval: true`.
